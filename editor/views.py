...
from .models import Track, Dot, SubDot, Topic, SubscriptionPlan, PaymentMethod, Subscription, Earnings, Editor
...
from django.contrib.auth.decorators import login_required
from django.contrib import messages
from .models import Dot, Editor

# TOGGLE SUBSCRIPTION (Subscribe/Unsubscribe to Dot)
# ===========================================
@login_required
...
def add_topic(request, subdot_id):
    ...
    if request.method == 'POST':
        try:
            logger.info(f"Processing add_topic form submission for subdot: {subdot.title}")
            ...
            logger.info(f"Form data received - Title: {title}, Content length: {len(content) if content else 0}, Code length: {len(code) if code else 0}")
            
            # Create new topic directly without using the form
            topic = Topic(
                subdot=subdot,
                title=title,
                content=content,
                code=code,
                timestamps=json.loads(timestamps) if timestamps and timestamps.strip() else None,
                order=Topic.objects.filter(subdot=subdot).count() + 1
            )
            ...
            if 'image' in request.FILES:
                ...
                if image:
                    logger.info(f"Image uploaded: {image.name}, Size: {image.size}")
                    topic.image = image
            ...
            if 'audio' in request.FILES:
                ...
                if audio:
                    logger.info(f"Audio uploaded: {audio.name}, Size: {audio.size}")
                    topic.audio = audio
            
            # Save the topic without setting created_by
            topic.save()
            logger.info(f"Topic saved successfully with ID: {topic.id}")
